    ...
    if (std::holds_alternative<std::unique_ptr<Type>>(decl_specifiers)) {
      ...
      if (auto *enum_decl = dynamic_cast<EnumDeclNode*>(decl.get())) {
        ...
        if (custom_types.find(enum_type_id) == custom_types.end()) {
          ...
          // Unnamed enums merely introduce enumeration constants.
...
enum_specifier: ENUM id_opt LEFT_CURLY enumerator_list comma_opt RIGHT_CURLY {
    ...
    for (const auto& enum_const : enum_list) {
      enums.push_back(std::make_unique<OptEnumConst>(enum_const->id, enum_const->value ? std::optional{enum_const->value->val} : std::nullopt));
    }
    auto type_id = decl_id ? decl_id->id : "";
...
enumerator: enum_const { $$ = $1; }
  | enum_const ASSIGN const_expr {
    auto enum_const = $1;
    enum_const->value = std::unique_ptr<IntConstExprNode>(dynamic_cast<IntConstExprNode*>($3.release()));
    $$ = std::move(enum_const);
  }
  ;
